...
-#}
  ...
  {%- if execute and not dbt_cortex_agent._mcp_api_integration_exists(api_integration) -%}
    {{ exceptions.raise_compiler_error(
        ...
        ~ "either as a first-class dbt model (recommended — gives you a real DAG edge "
        ~ "via cortex_mcp_api_integration_name(ref(...)) instead of a bare name string):\n\n"
        ~ "    {{ config(\n"
