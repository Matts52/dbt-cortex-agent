{% macro _mcp_api_integration_ddl(
    integration_name,
    allowed_prefixes,
...
{#-
--  Validates arguments and builds the `CREATE API INTEGRATION` DDL text for a
--  Snowflake external-MCP API integration. Pure string builder — does not
--  execute anything. Shared by the `create_mcp_api_integration` run-operation
--  and the `cortex_mcp_api_integration` materialization so both produce
--  identical DDL from the same validation rules.
--
--  See `create_mcp_api_integration` (this file) and
--  `macros/relations/cortex_mcp_api_integration/create.sql` for callers.
-#}
  ...
  {%- if auth_type_upper not in ['OAUTH2', 'OAUTH_DYNAMIC_CLIENT'] -%}
    {{ exceptions.raise_compiler_error(
        "_mcp_api_integration_ddl: unsupported auth_type '" ~ auth_type ~ "'. "
  ...
  {%- if auth_type_upper == 'OAUTH_DYNAMIC_CLIENT' and oauth_resource_url is none -%}
    {{ exceptions.raise_compiler_error(
        "_mcp_api_integration_ddl: 'oauth_resource_url' is required when auth_type='OAUTH_DYNAMIC_CLIENT'."
    ) }}
  {%- endif -%}
  ...
  {%- if auth_type_upper == 'OAUTH2' -%}
    {%- if oauth_client_id is none or oauth_client_secret is none
      ...
      {{ exceptions.raise_compiler_error(
          "_mcp_api_integration_ddl: auth_type='OAUTH2' requires "
...
--      }'
--
--  Prefer the `cortex_mcp_api_integration` materialization instead of this
--  operation for anything that should show up in the DAG/manifest — see
--  macros/materializations/cortex_mcp_api_integration.sql. This operation
--  remains for one-off, admin-run bootstrapping outside of `dbt build`
--  (e.g. from a session that only has ACCOUNTADMIN for the duration of the
--  bootstrap) and defaults to `or replace` to preserve existing behavior for
--  callers already scripting against it.
--
--  Parameters:
...
-#}
  ...
  {%- set ddl = dbt_cortex_agent._mcp_api_integration_ddl(
